...
import {
  QuoteHistory,
  QuoteHistoryAction,
} from './entities/quote-history.entity';
...
export class QuotesService implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.quotesRepo.query(
      `CREATE SEQUENCE IF NOT EXISTS quote_number_seq START 1`,
    );
    const [lastQuote] = await this.quotesRepo.find({
  ...
  private async generateQuoteNumber(): Promise<string> {
    ...
    const result = await this.quotesRepo.query(
  ...
  async findAll(paginationDto: PaginationDto): Promise<PaginatedResult<Quote>> {
    ...
    const where = search ? [{ quoteNumber: ILike(`%${search}%`) }] : {};
  ...
  async update(
    ...
    if (dto.status && dto.status !== quote.status)
      changes.status = { old: quote.status, new: dto.status };
    if (dto.notes !== undefined && dto.notes !== quote.notes)
      changes.notes = { old: quote.notes, new: dto.notes };
    if (dto.validUntil) {
      const newValid = new Date(dto.validUntil).toISOString().split('T')[0];
      const oldValid = quote.validUntil
        ? new Date(quote.validUntil).toISOString().split('T')[0]
        : null;
      if (newValid !== oldValid)
        changes.validUntil = { old: oldValid, new: newValid };
    }
    ...
    return await this.quotesRepo.manager.transaction(async (manager) => {
      ...
      const updatedQuote = (await manager.save(Quote, {
        ...
        totalValue:
          newItems.length > 0
            ? newItems.reduce((s, i) => s + Number(i.subtotal), 0)
            : quote.totalValue,
        items: undefined,
      })) as Quote;
  ...
  async generatePdf(id: string, res: Response): Promise<void> {
    ...
    const companyAddress = await this.settingsService.getValue(
      'company_address',
      'Carrera 106 # 15 - 25 Manzana 14 - Bodega 92 zona franca Fontibón',
    );
    const companyPhones = await this.settingsService.getValue(
      'company_phones',
      '3115111439 / 4238000 EXT: 33373 - 33374 - 33320 - 33324 - 33290.',
    );
    const companyCity = await this.settingsService.getValue(
      'company_city',
      'Bogotá D.C.',
    );
          ...
          table: {
            ...
            body: [
              [
                {
                  image:
                    logoBase64 ||
                    'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=', // transparent 1x1 fallback
                  width: 120,
                  rowSpan: 4,
                  alignment: 'center',
                  margin: [0, 10, 0, 0],
              ...
              [
                ...
                {
                  text: new Date(quote.createdAt).toISOString().split('T')[0],
                  alignment: 'center',
                  fontSize: 8,
                },
                '',
          ...
          table: {
            ...
            body: [
              ...
              [
                ...
                {
                  text: 'INSTRUMENTO\n(Detallar Marca, Modelo, Cap. Máxima, División escala)',
                  bold: true,
                  alignment: 'center',
                  fillColor: '#b30000',
                  color: 'white',
              ...
              ...quote.items.map((item, index) => [
                ...
                {
                  text: `$ ${Number(item.unitPrice).toLocaleString('es-CO')}`,
                  alignment: 'right',
                },
                {
                  text: `$ ${Number(item.subtotal).toLocaleString('es-CO')}`,
                  alignment: 'right',
                },
              ]),
              // Empty rows to match design if needed
              ...Array.from({
                length: Math.max(0, 12 - quote.items.length),
              }).map((_, i) => [
                {
                  text: (quote.items.length + i + 1).toString(),
                  alignment: 'center',
                },
                { text: '*' },
                { text: '*' },
              ...
              [
                {
                  text: [
                    { text: 'Observaciones:\n', bold: true },
                    {
                      text:
                        quote.notes ||
                        'El cliente debe garantizar que sus instrumentos cuenten con los mantenimientos respectivos antes de realizar la calibración.\n\n',
                      fontSize: 7,
                    },
                    {
                    ...
                    {
                      text: 'Nuestro certificado puede ser consultado en la página web: https://onac.org.co/certificados/21-LAC-011.pdf o solicitada por medio de nuestros canales de atención',
                      fontSize: 7,
                      color: 'blue',
                ...
                {
                  text: `$ ${Number(quote.totalValue).toLocaleString('es-CO')}`,
                  alignment: 'right',
                },
              ],
              [
              ...
              [
                ...
                {
                  text: `$ ${(Number(quote.totalValue) * 0.19).toLocaleString('es-CO')}`,
                  alignment: 'right',
                },
              ],
              [
                ...
                {
                  text: `$ ${(Number(quote.totalValue) * 1.19).toLocaleString('es-CO')}`,
                  alignment: 'right',
                },
              ],
            ],
        ...
        // CONDICIONES COMERCIALES
        {
          table: {
            ...
            body: [
              ...
              [
                { text: 'Vigencia de la oferta:', bold: true },
                {
        ...
        // LEGAL TEXTS
        {
          table: {
            ...
            body: [
              ...
              [
                {
                  text: 'INFORMACIÓN GENERAL PARA LA PRESTACIÓN DEL SERVICIO DE CALIBRACIÓN',
                  bold: true,
                  alignment: 'center',
                  fillColor: '#b30000',
                  color: 'white',
  ...
  async importFromExcel(
    ...
    for (const row of data) {
      ...
      // Map Tracking fields
      tracking.fechaPactadaServicio =
        parseDate(row['Fecha pactada de la Prestación del Servicio']) ||
        tracking.fechaPactadaServicio;
      tracking.idOrdenTrabajo = row['ID Orden de Trabajo']
        ? String(row['ID Orden de Trabajo']).trim()
        : tracking.idOrdenTrabajo;
      tracking.idRequisicion = row['ID Requisición']
        ? String(row['ID Requisición']).trim()
        : tracking.idRequisicion;
      tracking.fechaReporte =
        parseDate(row['Fecha de Reporte']) || tracking.fechaReporte;
      tracking.idReporteServicio = row['ID Reporte de Servicio']
        ? String(row['ID Reporte de Servicio']).trim()
        : tracking.idReporteServicio;
      tracking.fechaRecepcionEquipos =
        parseDate(row['Fecha de Recepción de Equipos']) ||
        tracking.fechaRecepcionEquipos;
      tracking.idRecepcionEquipos = row['ID Recepción de Equipos']
        ? String(row['ID Recepción de Equipos']).trim()
        : tracking.idRecepcionEquipos;
      tracking.fechaEntregaOc =
        parseDate(row['Fecha de entrega OC']) || tracking.fechaEntregaOc;
      tracking.idOrdenCompra = row['ID Orden de Compra']
        ? String(row['ID Orden de Compra']).trim()
        : tracking.idOrdenCompra;
      tracking.fechaIngresoLabExterno =
        parseDate(row['Fecha de Ingreso a Lab. Externo']) ||
        tracking.fechaIngresoLabExterno;
      tracking.laboratorioExterno = row['Laboratorio Externo']
        ? String(row['Laboratorio Externo']).trim()
        : tracking.laboratorioExterno;
      tracking.fechaEntregaEquipoLabExterno =
        parseDate(row['Fecha de Entrega del Equipo Lab Externo']) ||
        tracking.fechaEntregaEquipoLabExterno;
      tracking.fechaRecogerEquipo =
        parseDate(row['Fecha de Recoger el Equipo']) ||
        tracking.fechaRecogerEquipo;
      tracking.fechaEntregaEquipoCliente =
        parseDate(row['Fecha de Entrega del Equipo al Cliente']) ||
        tracking.fechaEntregaEquipoCliente;
      tracking.idReporteEntregaServicios = row[
        'ID Reporte Entrega de Servicios'
      ]
        ? String(row['ID Reporte Entrega de Servicios']).trim()
        : tracking.idReporteEntregaServicios;
      tracking.fechaReporteEntregaServicio =
        parseDate(row['Fecha de Reporte Entrega de Servicio']) ||
        tracking.fechaReporteEntregaServicio;
      tracking.fechaEmisionCertificado =
        parseDate(row['Fecha de Emisión del Certificado']) ||
        tracking.fechaEmisionCertificado;
      tracking.idCertificado = row['ID Certificado']
        ? String(row['ID Certificado']).trim()
        : tracking.idCertificado;
      tracking.fechaEntregaCertificado =
        parseDate(row['Fecha de Entrega Certificado']) ||
        tracking.fechaEntregaCertificado;
      tracking.idFactura = row['ID Factura']
        ? String(row['ID Factura']).trim()
        : tracking.idFactura;
      tracking.fechaFactura =
        parseDate(row['Fecha de Factura']) || tracking.fechaFactura;
      tracking.fechaPago =
        parseDate(row['Fecha de Pago']) || tracking.fechaPago;
      tracking.comprobanteEgreso = row['Comprobante de Egreso']
        ? String(row['Comprobante de Egreso']).trim()
        : tracking.comprobanteEgreso;
  ...
  async generateTechnicalPdf(id: string, res: Response): Promise<void> {
              ...
              [
                {
                  text: [
                    ...
                    {
                      text: 'Puede consultar nuestro alcance acreditado 21-LAC-011 en el directorio de acreditados de ONAC o en el siguiente link: ',
                      italics: true,
                    },
                    {
                    ...
                    {
                      text: 'Consulte nuestras condiciones comerciales en el siguiente Link: ',
                      italics: true,
                    },
                    {
                      text: 'https://drive.google.com/file/d/1y2Jjqk3IZAB3AqOAWoZToTiNqL-pvvvo/view?usp=drive_link',
                      color: 'blue',
                      decoration: 'underline',
                    },
                  ],
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${quote.quoteNumber}-tecnica.pdf"`,
    );
    pdfDoc.pipe(res);
    pdfDoc.end();
  }
  ...
  async generateTechnicalExcel(id: string, res: Response): Promise<void> {
    ...
    const sheet = workbook.addWorksheet('Cotización Técnica', {
    ...
    sheet.getCell('E3').value = new Date(quote.createdAt)
      .toISOString()
      .split('T')[0];
    ...
    const headers = [
      ...
      'SERVICIO',
      'EQUIPO',
      'INTERVALO DE MEDICIÓN CAP MÁXIMA',
      'DIVISIÓN DE ESCALA',
      'MARCA',
      'MODELO',
      'SERIE',
    ...
    headers.forEach((h, i) => {
      ...
      cell.font = {
        bold: true,
        color: { argb: i < 5 ? 'FFFFFFFF' : 'FF000000' },
      };
      cell.fill = {
        type: 'pattern',
        pattern: 'solid',
    ...
    sheet.getCell(`B${currentRow}`).value = quote.validUntil
      ? new Date(quote.validUntil).toLocaleDateString('es-CO')
      : '';
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${quote.quoteNumber}-tecnica.xlsx"`,
    );
